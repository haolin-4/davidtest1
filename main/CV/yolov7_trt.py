...
class YoloTRT():
    def __init__(self, library, engine, conf, iou, yolo_ver='v7', obj_filter=[]):
    ...
    def Inference(self, img):
        ...
        for i in range(self.batch_size):
            ...
            result = self.PostProcess(output[i * self.LEN_ALL_RESULT: (i + 1) * self.LEN_ALL_RESULT], origin_h, origin_w)
            
        # det_res = []
        # for j in range(len(result_boxes)):

        #     if result_classid[j] in self.obj_filter:
        #         continue

        #     det = dict()
        #     det["class"] = int(result_classid[j])
